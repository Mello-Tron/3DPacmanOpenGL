...
public:
	...
	void init_texture_map()
	{
		...
		int TexRight = MyLoadCubeMapBitmap("skybox\\space\\sidem.bmp", GL_TEXTURE_CUBE_MAP_POSITIVE_X);
		int TexLeft = MyLoadCubeMapBitmap("skybox\\space\\sideb.bmp", GL_TEXTURE_CUBE_MAP_NEGATIVE_X);
		int TexTop = MyLoadCubeMapBitmap("skybox\\space\\up.bmp", GL_TEXTURE_CUBE_MAP_POSITIVE_Y);
		int TexBottom = MyLoadCubeMapBitmap("skybox\\space\\bottom.bmp", GL_TEXTURE_CUBE_MAP_NEGATIVE_Y);
		int TexFront = MyLoadCubeMapBitmap("skybox\\space\\sider.bmp", GL_TEXTURE_CUBE_MAP_POSITIVE_Z);
		int TexBack = MyLoadCubeMapBitmap("skybox\\space\\sidel.bmp", GL_TEXTURE_CUBE_MAP_NEGATIVE_Z);
	...
	{		
		start_shader();
	        ...
	        glUniformMatrix4fv(uniforms[0], 1, GL_TRUE, modelview * Scale(10000.0, 9000.0, 10000.0) * RotateZ(theta[2]) * RotateX(theta[0]) * RotateY(theta[1]) );
			GL_CHECK_ERRORS
